...
"""
通用台词本流水线（可复用于每一批）：
- 下载 subsaga SRT 字幕（series-1/{ep_no}-{slug}.srt，POST）
- 翻译（dict_zh + Google gtx）
- ffmpeg 逐句截图
- 生成旧版 HTML（含章节 line-card）+ 输出 JSON 数据

用法：编辑下方 EPISODES 为本批要处理的剧集，然后 `python3.11 tools/pipeline.py`
"""
import re
import json
import time
...
# (slug, 英文标题, 中文, 集号, 腾讯vid)
EPISODES = [
    ("worm", "Worm", "虫子", 11, "s3549lzx669"),
    ("boat", "Boat", "船", 12, "c3535xnipen"),
    ("house", "House", "房子", 13, "k3549di8kbx"),
    ("lion", "Lion", "狮子", 14, "x3363n5c6xs"),
    ("apple", "Apple", "苹果", 15, "r3327ldpxbg"),
]
...
def google_translate(text: str, attempts: int = 6) -> str:
    last_err = None
...
def main():
    ...
    for ep, title, zh, ep_no, vid in EPISODES:
        ...
        if not video_path.exists():
            print(f"  视频缺失: {video_path}，请先下载 {vid}")
            continue
